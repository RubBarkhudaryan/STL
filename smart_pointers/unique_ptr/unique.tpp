...
rub::unique_ptr<T, Deleter>::unique_ptr(rub::unique_ptr<T, Deleter> &&other) noexcept :
	ptr(other.release()), del(std::move(other.get_deleter()))
{}
...
rub::unique_ptr<T, Deleter>&	rub::unique_ptr<T, Deleter>::operator=(rub::unique_ptr<T, Deleter>&& other) noexcept
{
	if (this != &other)
	{
		...
		this->del = std::move(other.get_deleter());
	}
	return (*this);
}
...
rub::unique_ptr<T[], Deleter>::unique_ptr(U *ptr) noexcept :
	ptr(ptr), del(Deleter{})
{}

template<typename T, typename Deleter>
template<typename U, typename>
rub::unique_ptr<T[], Deleter>::unique_ptr(U *ptr, Deleter del) noexcept :
	ptr(ptr), del(del)
{}

template<typename T, typename Deleter>
template<typename U, typename E, typename>
rub::unique_ptr<T[], Deleter>::unique_ptr(rub::unique_ptr<U[], E>&& other) noexcept :
	ptr(other.release()), del(std::move(other.get_deleter()))
{}
...
template<typename T, typename Deleter>
T&	rub::unique_ptr<T[], Deleter>::operator[](std::size_t ind) const
{
	return (this->ptr[ind]);
}
